...
We fine-tune \textbf{DeBERTa-V3-base}~\cite{he2021debertav3} (\texttt{microsoft/deberta-v3-base}, 86M parameters) for binary sequence classification.
...
\paragraph{Differential learning rates.} The classifier head is updated at a higher rate ($1 \times 10^{-4}$) than the backbone ($2 \times 10^{-5}$). The backbone weights are already near-optimal from pre-training; large updates risk destroying the representations that give DeBERTa-V3 its advantage.
...
We expect \texttt{deberta-v3-base} to comfortably exceed the RoBERTa-base baseline while remaining efficient to fine-tune on a single GPU.
